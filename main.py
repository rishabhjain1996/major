...
staff_lower, staff_upper, staff_thresh = 50, 150, 0.77                                  #MAKING CHANGES CUZ TLE
sharp_lower, sharp_upper, sharp_thresh = 50, 150, 0.70
flat_lower, flat_upper, flat_thresh = 50, 150, 0.77                                 #50, 150, 0.77
quarter_lower, quarter_upper, quarter_thresh = 70, 90, 0.70                             #50, 150, 0.70
half_lower, half_upper, half_thresh = 70, 90, 0.70                                     #50, 150, 0.70
whole_lower, whole_upper, whole_thresh = 70, 90, 0.70                                  #50, 150, 0.70


def locate_staff(img, templates, start, stop, threshold):
...
def locate_images(img, start, stop, threshold,str):
    locations, scale = fity(img, start, stop, threshold,str)
    img_locations = []
    w, h = SAMy, SAMx
    w *= scale
    h *= scale
    img_locations.append([Rectangle(pt[0], pt[1], w, h) for pt in zip(*locations[0][::-1])])
    return img_locations
...
if __name__ == "__main__":
    ...
    staff_recs = locate_staff(img_gray, staff_imgs, staff_lower, staff_upper, staff_thresh)
    ...
    sharp_recs = locate_staff(img_gray, sharp_imgs,sharp_lower, sharp_upper, sharp_thresh)
    ...
    flat_recs = locate_staff(img_gray,flat_imgs,flat_lower, flat_upper, flat_thresh)
    ...
    quarter_recs = locate_images(img_gray, quarter_lower, quarter_upper, quarter_thresh,"quarter")
    ...
    half_recs = locate_images(img_gray, half_lower, half_upper, half_thresh,"half")
    ...
    whole_recs = locate_images(img_gray, whole_lower, whole_upper, whole_thresh,"whole")
